...
void Invalid_edge_preprocessing_3::add () {
  ...
  for (const Vertex& i : cubic_model.instance.customers) 
    for (const Vertex& j : cubic_model.instance.customers) {
      invalidEdge = true;
        ...
        if (f0[f].id == cubic_model.instance.depot.id || pred[f] != f) 
          for (r = 0; r < sf0; r++) 
            ...
            if (f0[r].id == cubic_model.instance.depot.id || pred[r] != r) {
              distance = cubic_model.instance.distances[f0[f].id][i.id] + cubic_model.instance.distances[i.id][j.id] + cubic_model.instance.distances[j.id][f0[r].id];
              if (distance * cubic_model.instance.vehicleFuelConsumptionRate <= cubic_model.instance.vehicleFuelCapacity && (times[f] + (distance/cubic_model.instance.vehicleAverageSpeed) + i.serviceTime + j.serviceTime + times[r] <= cubic_model.instance.timeLimit)) { 
                invalidEdge = false;
                f = sf0;
                break;
              }
