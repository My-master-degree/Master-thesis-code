...
#include "models/gvrp_models/cplex/lh_model/subcycle_user_constraint.hpp"
...
Subcycle_user_constraint::Subcycle_user_constraint (LH_model& lh_model_) : User_constraint (lh_model_) {}
...
void Subcycle_user_constraint::main() {
  ...
  //setup
  const size_t sc0 = lh_model.c0.size(),
        sf0 = lh_model.f0.size();
  DSU dsu (sc0);
  IloEnv env = getEnv();
  IloExpr lhs(env);
