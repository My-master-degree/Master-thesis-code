...
Subcycle_user_constraint::Subcycle_user_constraint (Matheus_model_5& matheus_model_5_) : User_constraint (matheus_model_5_) {}
...
//    return;
  ...
  for (int i = 0; i < sc0; ++i) {
    ...
    getValues(x_vals[i], matheus_model_5.x[i]);
    for (int f = 0; f < s_f; ++f) {
      y_vals[i][f] = Matrix2DVal (env, s_f);
      for (int r = 0; r < s_f; ++r) {
        y_vals[i][f][r] = IloNumArray (env, sc0, 0, 1, IloNumVar::Float);
        getValues(y_vals[i][f][r], matheus_model_5.y[i][f][r]);
      }
    }
  }
  //creating nodes
  ...
  for (const unordered_set<int>& S : components) 
    if (!S.count(0)) {
      ...
      for (int i = 0; i < sc0; ++i) 
        if (!S.count(i)) 
          for (int j : S) {
            lhs += matheus_model_5.x[i][j];
            for (int f = 0; f < s_f; ++f)
              for (int r = 0; r < s_f; ++r)
                lhs += matheus_model_5.y[i][f][r][j];
          }
      //rhs
      const int sS = S.size();
      ...
      int j = 0;
      for (int i : S) {
        vertices[j] = matheus_model_5.c0[i];
        ++j;
      }
      vertices[j] = matheus_model_5.c0[0];
      //get n routes lbs
      const auto& closestsTimes = calculateClosestsGVRPCustomers(matheus_model_5.gvrpReducedGraphTimes, vertices);
      //get mst
      improvedMSTNRoutesLB = int(ceil(calculateGvrpLBByImprovedMSTTime(vertices, closestsTimes, matheus_model_5.gvrpReducedGraphTimes)/matheus_model_5.instance.timeLimit));
      //bin packing
      bppNRoutesLB = calculateGVRP_BPP_NRoutesLB(matheus_model_5.instance, vertices, closestsTimes, matheus_model_5.BPPTimeLimit);
      maxNRoutes = max(improvedMSTNRoutesLB, bppNRoutesLB);
      if (improvedMSTNRoutesLB == maxNRoutes) 
        ++matheus_model_5.nImprovedMSTNRoutesLB;
      if (bppNRoutesLB == maxNRoutes) 
        ++matheus_model_5.nBPPNRoutesLB;
      lhs -= maxNRoutes;
      try {
        add(lhs >= 0).end();
